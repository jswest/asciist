...
class Asciist {
	...
	handleImageBuffer(data) {
		...
		return new Promise((resolve, reject) => {
			...
			this.state.grayPixels = grayPixels;
			resolve();
		});
	}
	...
	asciizeImage() {
		const { bins, chars, grayPixels } = this.state;
		return new Promise((resolve, reject) => {
			let outputString = "";
			for (let y = 0; y < grayPixels.length; y++) {
				for (let x = 0; x < grayPixels[y].length; x++) {
	...
	writeToFile() {
		...
		return new Promise((resolve, reject) => {
			if (saveAsImage) {
				...
				rows.forEach((r, i) => {
					r.split("").forEach((l, m) => {
						context.fillStyle = `rgb(${pixels[
							Math.floor((i * r.length + m) / 2)
						].join(",")})`;
						context.fillText(l, m * 10, i * 20 + 20);
					});
				});
